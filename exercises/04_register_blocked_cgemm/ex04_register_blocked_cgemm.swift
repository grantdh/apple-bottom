...
func accelerateCGEMM(A: UnsafePointer<SIMD2<Float>>, B: UnsafePointer<SIMD2<Float>>,
    ...
    // SIMD2<Float> has identical memory layout: [real, imag] contiguous.
    // UnsafeRawPointer bridges the type system without copying.
    cblas_cgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                Int32(M), Int32(N), Int32(K),
                &alpha,
                UnsafeRawPointer(A), Int32(K),
                UnsafeRawPointer(B), Int32(N),
                &beta,
                UnsafeMutableRawPointer(C), Int32(N))
}
