...
ABStatus ab_init(void) {
    if (g_context) return AB_OK;
    init_timing();
    g_context = [[ABContextImpl alloc] init];
    return g_context ? AB_OK : AB_ERROR_NO_DEVICE;
}

void ab_shutdown(void) { [ABContextImpl shutdown]; memset(&g_stats, 0, sizeof(g_stats)); }
const char* ab_device_name(void) { ABContextImpl* ctx = [ABContextImpl shared]; return ctx ? ctx.device.name.UTF8String : "Unknown"; }
...
ABMatrix ab_matrix_create(int rows, int cols) {
    ...
    if (!m) return NULL;
    m->rows = rows; m->cols = cols; m->count = (size_t)rows * cols; m->uploaded = false;
    m->buffer = [ctx.device newBufferWithLength:m->count * sizeof(DDFloat) options:MTLResourceStorageModeShared];
    if (!m->buffer) { free(m); return NULL; }
    return m;
}

void ab_matrix_destroy(ABMatrix m) { if (m) { m->buffer = nil; free(m); } }
void ab_matrix_dims(ABMatrix m, int* rows, int* cols) { if (m) { if (rows) *rows = m->rows; if (cols) *cols = m->cols; } }
...
ABStatus ab_matrix_upload(ABMatrix m, const double* data, bool parallel) {
    ...
    if (parallel && m->count > 1000) convert_fp64_to_dd_parallel(data, dst, m->count);
    else for (size_t i = 0; i < m->count; i++) dst[i] = fp64_to_dd(data[i]);
    m->uploaded = true;
...
ABStatus ab_matrix_download(ABMatrix m, double* data, bool parallel) {
    ...
    if (parallel && m->count > 1000) convert_dd_to_fp64_parallel(src, data, m->count);
    else for (size_t i = 0; i < m->count; i++) data[i] = dd_to_fp64(src[i]);
...
ABStatus ab_matrix_zero(ABMatrix m) {
    ...
    [encoder endEncoding]; [cmdBuf commit]; [cmdBuf waitUntilCompleted];
    return AB_OK;
}
...
ABStatus ab_zgemm(ABMatrix Ar, ABMatrix Ai, ABMatrix Br, ABMatrix Bi, ABMatrix Cr, ABMatrix Ci) {
    ...
    if (!T1 || !T2 || !T3 || !T4) { ab_matrix_destroy(T1); ab_matrix_destroy(T2); ab_matrix_destroy(T3); ab_matrix_destroy(T4); return AB_ERROR_ALLOC_FAILED; }
    ...
    ab_matrix_destroy(T1); ab_matrix_destroy(T2); ab_matrix_destroy(T3); ab_matrix_destroy(T4);
    stats_add_zgemm();
    return AB_OK;
}
...
ABSession ab_session_create(void) { return calloc(1, sizeof(struct ABSession_s)); }
void ab_session_destroy(ABSession s) { if (!s) return; for (int i = 0; i < s->count; i++) ab_matrix_destroy(s->matrices[i]); free(s); }
...
ABMatrix ab_session_get(ABSession s, const char* name) {
    ...
    for (int i = 0; i < s->count; i++) if (strcmp(s->names[i], name) == 0) return s->matrices[i];
    return NULL;
}

ABStatus ab_session_upload(ABSession s, const char* name, const double* data) { ABMatrix m = ab_session_get(s, name); return m ? ab_matrix_upload(m, data, true) : AB_ERROR_INVALID_ARG; }
ABStatus ab_session_download(ABSession s, const char* name, double* data) { ABMatrix m = ab_session_get(s, name); return m ? ab_matrix_download(m, data, true) : AB_ERROR_INVALID_ARG; }
ABStatus ab_session_dgemm(ABSession s, const char* A, const char* B, const char* C) { ABMatrix mA = ab_session_get(s, A), mB = ab_session_get(s, B), mC = ab_session_get(s, C); return (mA && mB && mC) ? ab_dgemm(mA, mB, mC) : AB_ERROR_INVALID_ARG; }
ABStatus ab_session_zgemm(ABSession s, const char* Ar, const char* Ai, const char* Br, const char* Bi, const char* Cr, const char* Ci) {
    ABMatrix mAr = ab_session_get(s, Ar), mAi = ab_session_get(s, Ai), mBr = ab_session_get(s, Br), mBi = ab_session_get(s, Bi), mCr = ab_session_get(s, Cr), mCi = ab_session_get(s, Ci);
    return (mAr && mAi && mBr && mBi && mCr && mCi) ? ab_zgemm(mAr, mAi, mBr, mBi, mCr, mCi) : AB_ERROR_INVALID_ARG;
}

ABStats ab_get_stats(void) { os_unfair_lock_lock(&g_stats_lock); ABStats copy = g_stats; os_unfair_lock_unlock(&g_stats_lock); return copy; }
void ab_reset_stats(void) { os_unfair_lock_lock(&g_stats_lock); memset(&g_stats, 0, sizeof(g_stats)); os_unfair_lock_unlock(&g_stats_lock); }
void ab_print_stats(void) {
    ABStats s = ab_get_stats();
    fprintf(stderr, "apple-bottom statistics:\n  Upload: %.2f ms\n  Download: %.2f ms\n  Kernel: %.2f ms\n  DGEMM: %llu\n  ZGEMM: %llu\n", s.upload_time_ms, s.download_time_ms, s.kernel_time_ms, s.dgemm_count, s.zgemm_count);
}
